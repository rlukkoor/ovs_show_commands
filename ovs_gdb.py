...
#
# Implements the GDB "dump_sync_info" command
#
class CmdNuageDumpSyncInfo(gdb.Command):
    """Dump all configured bridges.
    Usage: dump_sync_info <bridge>
    """
    def __init__(self):
        super(CmdNuageDumpSyncInfo, self).__init__("dump_sync_info",
                                            gdb.COMMAND_DATA)
    ...
    def invoke(self, arg, from_tty):
        ...
        if len(arg_list) > 0:
            ...
            return
        vds = get_global_variable('dhcp_sync_info')
        if vds is None:
            return

        # sync = all_dhcp_sync['']
        # vds = sync.cast(gdb.lookup_type('struct vrs_dhcp_sync').pointer())
        print("-------- start ---------")

        if vds['bfd']:
            print("hello")
        else:
            print("vds bfd: no vds bfd yet")

        print("last_tx_timestamp: {}\n vds_init_timestamp: {}\n sync_gen_id: {}\n tx_seq_num: {}\n"
            "rx_seq_num: {}".
            format(vds['last_tx_timestamp'], vds['vds_bfd_init_timestamp'], vds['sync_gen_id'],
                vds['tx_seq_num'], vds['rx_seq_num']))

        if vds['is_cfg_master']:
            print("cfg_master: yes")
        else:
            print("cfg_master: no")

        if vds['ack_pending']:
            print("ack_pending: yes")
        else:
            print("ack_pending: no")

        if vds['sync_state'] == 0:
            print("sync_state: none")
        elif vds['sync_state'] == 1:
            print("sync_state: in_prog")
        elif vds['sync_state'] == 2:
            print('sync_state: in_audit')
        elif vds['sync_state'] == 3:
            print("sync_state: complete")
        else:
            print("sync_state: invalid")

        print("vds->temporal_list: {}".
            format(vds['temporal_list']))

        # desn = vds['dhcp_entry_sync_node']
        # dhcp = desn.cast(gdb.lookup_type('struct dhcp_entry').pointer())

        tsi = vds['tx_start_iter']
        dhcp_s = tsi.cast(gdb.lookup_type('struct dhcp_entry'))

        tei = vds['tx_end_iter']
        dhcp_e = tei.cast(gdb.lookup_type('struct dhcp_entry'))

        print("vds->tx_start_iter: {}\n vds->tx_start_iter.next: {}, {}\n"
            "vds->tx_start_iter.prev: {}, {}\n vds->tx_end_iter: {}\n vds->tx_end_iter.next: {}, {}\n"
            "vds->tx_end_iter.prev: {}, {}".
            format(
                # temp_list['temporal_list'], 
                tsi['sync_node'], tsi['sync_node']['next'], 
                tsi['sync_node']['next'], tsi['sync_node']['prev'],
                tsi['sync_node']['prev'], tei['sync_node'],
                tei['sync_node']['next'], tei['sync_node']['next'], 
                tei['sync_node']['prev'], tei['sync_node']['prev']))

        print("------ start iter entry -------")
        print("evpn: {}\n this_node: {}\n next: 0x{}\n prev: 0x{}".
            format(hex(int(tsi['evpn_id'])), tsi['sync_node'],
                tsi['sync_node']['next'], tsi['sync_node']['prev']))
        if tsi['dl_src']:
            sys.stdout.write("mac: {} | ".
                format(tsi['dl_src']))
        else:
            sys.stdout.write("mac: ff:ff:ff:ff:ff:ff | ")

        sys.stdout.write("ip: {} | ".
            format(str(netaddr.IPAddress(socket.ntohl(int(dhcp_s['ip']))))))

        print("flags: ")

        dhcp_entry_on_sync_list = 0x2
        dhcp_entry_del_pending = 0x1
        dhcp_entry_iter = 0x4
        if tsi['flags'] & dhcp_entry_on_sync_list:
            print("\tON SYNC_LIST: yes")
        else:
            print("\tON SYNC_LIST: no")

        if tsi['flags'] & dhcp_entry_del_pending:
            print("\tDEL_PENDING: yes")
        else:
            print("\tDEL_PENDING: no")

        if tsi['flags'] & dhcp_entry_iter:
            print("\tITER SET: yes")
        else:
            print("\tITER SET: no")

        print("------ start iter entry complete -------\n")

        print("------ end iter entry -------")
        print("evpn: {}\n this_node: {}\n next: 0x{}\n prev: 0x{}".
            format(hex(int(tei['evpn_id'])), tei['sync_node'],
                tei['sync_node']['next'], tei['sync_node']['prev']))
        if tei['dl_src']:
            sys.stdout.write("mac: {} | ".
                format(tei['dl_src']))
        else:
        ...
        sys.stdout.write("ip: {} | ".
            format(str(netaddr.IPAddress(socket.ntohl(int(tei['ip']))))))

        print("flags: ")

        if tei['flags'] & dhcp_entry_on_sync_list:
            print("\tON SYNC_LIST: yes")
        else:
            print("\tON SYNC_LIST: no")

        if tei['flags'] & dhcp_entry_del_pending:
            print("\tDEL_PENDING: yes")
        else:
            print("\tDEL_PENDING: no")

        if tei['flags'] & dhcp_entry_iter:
            print("\tITER SET: yes")
        else:
            print("\tITER SET: no")

        print("------ end iter entry complete -------\n")

        print("--------temporal list start ---------")

        num = 1
        for anode in ForEachLIST(vds['temporal_list'], 
                                "struct dhcp_entry", "sync_node"):
            ...
            print("entry #: {}\n evpn: {}\n gen_id: {}\n this_node: {}\n next: 0x{}\n prev: 0x{}".
                format(num, hex(int(anode['evpn_id'])), anode["gen_id"], hex(id(anode['sync_node'])), 
                    anode['sync_node']['next'], anode['sync_node']['prev']))
            ...
            sys.stdout.write("ip: {} | ".
                format(str(netaddr.IPAddress(socket.ntohl(int(anode['ip']))))))
            ...
            # dhcp_entry_del_pending = 0x1
            # dhcp_entry_iter = 0x4
            if anode['flags'] & dhcp_entry_on_sync_list:
                print("\tON SYNC_LIST\t: yes")
            else:
                print("\tON SYNC_LIST\t: no")

            if anode['flags'] & dhcp_entry_del_pending:
                print("\tDEL_PENDING\t: yes")
            else:
                print("\tDEL_PENDING\t: no")

            if anode['flags'] & dhcp_entry_iter:
                print("\tITER SET\t: yes\n")
            else:
                print("\tITER SET\t: no\n")
